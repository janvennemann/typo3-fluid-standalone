...
class TemplateParser {
	...
	protected function buildObjectTree($splitTemplate, $context) {
		...
		$state = new \TYPO3\Fluid\Core\Parser\ParsingState();
        $state->injectVariableContainer(new \TYPO3\Fluid\Core\ViewHelper\TemplateVariableContainer());
	...
	protected function initializeViewHelperAndAddItToStack(\TYPO3\Fluid\Core\Parser\ParsingState $state, $namespaceIdentifier, $methodIdentifier, $argumentsObjectTree) {
        ...
        $viewHelperClassName = $this->resolveViewHelperName($namespaceIdentifier, $methodIdentifier);
		...
		$currentViewHelperNode = new \TYPO3\Fluid\Core\Parser\SyntaxTree\ViewHelperNode($viewHelper, $argumentsObjectTree);
	...
	protected function objectAccessorHandler(\TYPO3\Fluid\Core\Parser\ParsingState $state, $objectAccessorString, $delimiter, $viewHelperString, $additionalViewHelpersString) {
		...
		if (strlen($objectAccessorString) > 0) {
			$node = new \TYPO3\Fluid\Core\Parser\SyntaxTree\ObjectAccessorNode($objectAccessorString);
	...
	protected function buildArgumentObjectTree($argumentString) {
		if (strpos($argumentString, '{') === FALSE && strpos($argumentString, '<') === FALSE) {
			return new \TYPO3\Fluid\Core\Parser\SyntaxTree\TextNode($argumentString);
		}
	...
	protected function arrayHandler(\TYPO3\Fluid\Core\Parser\ParsingState $state, $arrayText) {
		$state->getNodeFromStack()->addChildNode(
			new \TYPO3\Fluid\Core\Parser\SyntaxTree\ArrayNode($this->recursiveArrayHandler($arrayText))
		);
	}
		...
		if (preg_match_all(self::$SPLIT_PATTERN_SHORTHANDSYNTAX_ARRAY_PARTS, $arrayText, $matches, PREG_SET_ORDER) > 0) {
			...
			foreach ($matches as $singleMatch) {
				...
				if (!empty($singleMatch['VariableIdentifier'])) {
					$arrayToBuild[$arrayKey] = new \TYPO3\Fluid\Core\Parser\SyntaxTree\ObjectAccessorNode($singleMatch['VariableIdentifier']);
					...
					$arrayToBuild[$arrayKey] = new \TYPO3\Fluid\Core\Parser\SyntaxTree\ArrayNode($this->recursiveArrayHandler($singleMatch['Subarray']));
				} else {
	...
	protected function textHandler(\TYPO3\Fluid\Core\Parser\ParsingState $state, $text) {
		$node = new \TYPO3\Fluid\Core\Parser\SyntaxTree\TextNode($text);
